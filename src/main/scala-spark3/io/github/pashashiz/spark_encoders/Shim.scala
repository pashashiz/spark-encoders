...
object Shim {
  ...
  /** Shim for StaticInvoke that has an extra constructor arg added in Spark 4.x. Databricks often
    * backports changes like this Databricks Runtimes running with older Spark versions. This change
    * was also backported to DBR 14, 15 and 16.
    */
  def staticInvoke(
  ...
  /** Shim for ClosureCleaner.clean that handles different implementations across Spark versions.
    *
    *   - Spark 3.x: Uses ClosureCleaner.clean(closure: AnyRef, checkSerializable: Boolean,
    *     cleanTransitively: Boolean): Unit
    *   - Spark 4.x: Introduces SparkClosureCleaner.clean(closure: AnyRef, checkSerializable:
    *     Boolean, cleanTransitively: Boolean): Unit which internally uses a new version of
